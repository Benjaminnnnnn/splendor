import { GameStatsUpdate, User, UserStats } from '../../../shared/types/user';
...
export interface UserRepository {
  ...
  updateUserStats(userId: string, update: GameStatsUpdate): Promise<UserStats>;
  getLeaderboard(limit?: number): Promise<Array<{ user: User; stats: UserStats }>>;
  getUserLeaderboardRank(userId: string): Promise<number | null>;
...
export class SqliteUserRepository implements UserRepository {
  ...
  async registerUser(params: RegisterUserParams): Promise<User> {
    const { id, username, email, passwordHash, createdAt } = params;
    ...
    this.db.transaction(() => {
      ...
      this.db.run('INSERT INTO notification_preferences (user_id) VALUES (?)', [id]);
    });
