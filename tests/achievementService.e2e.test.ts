...
async function registerUser(request: APIRequestContext, username: string, email: string, password: string) {
...
async function primeAuth(page: Page, user: User) {
  await page.addInitScript((storedUser) => {
    localStorage.setItem('splendor_user', JSON.stringify(storedUser));
  }, user);
}
...
test.describe('Achievement service e2e', () => {
  test('evaluates stats, persists unlocks, and renders profile badges', async ({ request, page }) => {
    ...
    const user = await registerUser(request, creds.username, creds.email, 'Password123!');
    await updateUserStats(request, user.id);

    // Act: evaluate achievements and load the profile page as the new user.
    const evaluation = await evaluateAchievements(request, user.id);
    await primeAuth(page, user);
  ...
  test('is idempotent across repeated evaluations and reloads', async ({ request, page }) => {
    ...
    const firstEval = await evaluateAchievements(request, user.id);
    ...
    const secondEval = await evaluateAchievements(request, user.id);
  ...
  test('applies speed thresholds at boundaries', async ({ request }) => {
    ...
    const fastUser = await registerUser(request, fastCreds.username, fastCreds.email, 'Password123!');
    await updateUserStats(
      request,
      fastUser.id,
      createStats({
    ...
    const fastEval = await evaluateAchievements(request, fastUser.id);
    ...
    const slowUser = await registerUser(request, slowCreds.username, slowCreds.email, 'Password123!');
    await updateUserStats(
      request,
      slowUser.id,
      createStats({
    ...
    const slowEval = await evaluateAchievements(request, slowUser.id);
  ...
  test('keeps achievements unlocked even if stats regress', async ({ request }) => {
    ...
    const initialEval = await evaluateAchievements(request, user.id);
    ...
    const afterRegression = await evaluateAchievements(request, user.id);
